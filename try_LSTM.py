...
# train an LSTM on trump_campaign speeches. compare performance to vanilla RNN.
...
def train_with_sgd(model, X_train, y_train, learning_rate=0.005, nepoch=1, evaluate_loss_after=5):
    ...
    for epoch in range(nepoch):
        ...
        if (epoch % evaluate_loss_after == 0):
            ...
            save_model_parameters_theano("data/gru-theano-%d-%d-%s.npz" % (model.hidden_dim, model.word_dim, time), model)
...
####################################################################### if no train, load prior model
    ...
    print "loading model parameters..."
    model = GRUTheano(vocabulary_size, hidden_dim=HIDDEN_DIM) # re-init from scratch
    load_model_parameters_theano(MODEL_FILE, model)
...
def generate_sentence_deprecated(model):
    ...
    while not new_sentence[-1] == word_to_index[sentence_end_token]:
        next_word_probs = model.predict(new_sentence) # forward_propagation
